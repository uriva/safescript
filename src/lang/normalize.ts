...
const serializeType = (t: TypeExpr): string => {
  switch (t.kind) {
    ...
    case "object":
      return `{${t.fields.map((f) => `${f.name}:${serializeType(f.type)}`).join(",")}}`;
  }
};
...
const serializeValue = (v: Value, state: RenameState): string => {
  switch (v.kind) {
    case "string":
      return `"${v.value.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\n/g, "\\n").replace(/\t/g, "\\t")}"`;
    case "number":
    ...
    case "object":
      return `{${v.fields.map((f) => `${f.key}:${serializeValue(f.value, state)}`).join(",")}}`;
    case "call":
      return v.args.length === 0
        ? `${v.op}()`
        : `${v.op}({${v.args.map((a) => `${a.key}:${serializeValue(a.value, state)}`).join(",")}})`;
    case "binary_op":
      return `(${serializeValue(v.left, state)}${v.op}${serializeValue(v.right, state)})`;
    case "unary_op":
...
const serializeStatements = (stmts: readonly Statement[], state: RenameState): string =>
  stmts.map((stmt) => {
    switch (stmt.kind) {
      case "assignment": {
      ...
      case "void_call":
        return stmt.call.args.length === 0
          ? `${stmt.call.op}()`
          : `${stmt.call.op}({${stmt.call.args.map((a) => `${a.key}:${serializeValue(a.value, state)}`).join(",")}})`;
      case "if_else": {
        const cond = serializeValue(stmt.condition, state);
        const then = serializeStatements(stmt.then, state);
        const elseStr = stmt.else ? ` else{${serializeStatements(stmt.else, state)}}` : "";
...
const serializeFn = (fn: FnDef, state: RenameState): string => {
  ...
  return `${fn.name}=(${params})${retType}=>{${body ? `${body};` : ""}return ${ret}}`;
};
